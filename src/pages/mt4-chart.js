...
const TradingChartMT = () => {
  const [data, setData] = useState([]);
  const [error, setError] = useState(null);
  const [currency, setCurrency] = useState("EURUSD"); // Default currency
  ...
  const toggleButton = async(event) => {
    ...
    if(isOn){
      ...
      console.log('off..');
      axios.post(`http://${apiUrl}/trading-bot-control/`, { action: 'stop' })
      .then(response => {
        console.log(response.data.message);
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(fetchData,600000); // Fetch data every 60 seconds (adjust as needed)
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${apiUrl}/api/trading-data-mt/`, {
